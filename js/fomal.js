...
function createtime1() {
  var grt = new Date("01/06/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  var ascll2 = [`NCC2-036`, `调用前置摄像头拍照成功，识别为「小猫咪」`, `Photo captured: `, ` 🐾 `];
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 欢迎光临，小猫咪", "color:white; background-color:#23c682", ""));

  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by ankh喵窝❀ %c 你正在访问ankh喵窝❀",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| ankhmeow🍓") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('ankhmeow🍓的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
l = ["非常抱歉，因为不可控原因，博客将于明天停止运营！", "微软垮了！", "你的电脑已经过载，建议立即关机！", "你知道吗？站长很喜欢你哦！", "一分钟有61秒哦"]
...
if (m == 1 && dd == 30) {//生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝我最可爱的恋人" + (y - 2006).toString() + "岁生日快乐！✨");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("06/03/2024 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://lskypro.acozycotage.net/Fomalhaut/badge/F小屋-科研摸鱼中.svg' title=''><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://lskypro.acozycotage.net/Fomalhaut/badge/F小屋-下班休息啦.svg' title=''><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://img2.imgtp.com/2024/03/07/vQgc56iF.jpg);
    --darkmode-bg:url(https://img2.imgtp.com/2024/03/05/PWGMAOt5.jpg);
    --mobileday-bg: url(https://img2.imgtp.com/2024/03/07/vQgc56iF.jpg);
    --mobilenight-bg: url(https://img2.imgtp.com/2024/03/05/PWGMAOt5.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
